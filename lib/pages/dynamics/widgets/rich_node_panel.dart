      ...
      if (i.type == 'RICH_TEXT_NODE_TYPE_TOPIC') {
        spanChildren.add(
          WidgetSpan(
            ...
            child: GestureDetector(
              onTap: () {
                String topic = i.text ?? i.origText ?? '';
      ...
      if (i.type == 'RICH_TEXT_NODE_TYPE_WEB') {
        ...
        spanChildren.add(
          WidgetSpan(
            ...
            child: GestureDetector(
              onTap: () {
                String? url = i.jumpUrl ?? i.origText;
          ...
          WidgetSpan(
            ...
            child: GestureDetector(
              onTap: () {
                try {
                  String dynamicId =
                      item?.basic?['comment_id_str'] ?? item?.idStr ?? '';
                  Get.toNamed(
                    '/webview',
                    parameters: {
                      'url':
                      ...
                      'type': 'vote',
                      'pageTitle': i.text != null && i.text!.isNotEmpty
                          ? '投票：${i.text}'
                          : '投票',
                    },
                  );
          ...
          WidgetSpan(
            ...
            child: GestureDetector(
              onTap: () {
                ...
                if (url == null || url.trim().isEmpty) {
                  if (i.rid != null && i.rid!.isNotEmpty) {
                    url =
                        'https://t.bilibili.com/lottery/h5/index/#/result?notice_id=${i.rid}';
                  }
                }
                if (url == null || url.trim().isEmpty) {
                  SmartDialog.showToast('未获取到抽奖信息');
                  return;
                }
                url = url.trim();
                if (url.startsWith('//')) {
          ...
          WidgetSpan(
            ...
            child: GestureDetector(
              onTap: () {
                String? url = i.jumpUrl;
                if (url == null || url.trim().isEmpty) {
                  SmartDialog.showToast('未获取到商品链接');
                  return;
                }
                url = url.trim();
                if (url.startsWith('//')) {
