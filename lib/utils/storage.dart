...
class GStorage {
  static const Set<String> exportableBoxNames = <String>{
  ...
  static Map toEncodableManually(Map data) {
    if (data.containsKey(specialKey)) {
      data[specialKey] = data[specialKey].map(
  ...
  static Future<String> exportAllSettings() async {
    return jsonEncode({
      StorageBoxName.setting: toEncodableManually(setting.toMap()),
      StorageBoxName.video: video.toMap(),
    });
  }
  ...
  static Future<void> importAllSettings(String data) async {
    ...
    if (decoded is! Map<String, dynamic>) {
      throw const FormatException('Settings backup must be a JSON object');
    }
    final Object? settingData = decoded[StorageBoxName.setting];
    final Object? videoData = decoded[StorageBoxName.video];
    if (settingData is! Map || videoData is! Map) {
      throw const FormatException('Settings backup roots must be objects');
    }
...
class SettingBoxKey {
  ...
  anonymity = 'anonymity',
  /// 推荐
  enableRcmdDynamic = 'enableRcmdDynamic', defaultRcmdType = 'defaultRcmdType', enableSaveLastData = 'enableSaveLastData', minDurationForRcmd = 'minDurationForRcmd', minLikeRatioForRecommend = 'minLikeRatioForRecommend', exemptFilterForFollowed = 'exemptFilterForFollowed', banWordForRecommend = 'banWordForRecommend',
  //filterUnfollowedRatio = 'filterUnfollowedRatio',
  applyFilterToRelatedVideos = 'applyFilterToRelatedVideos', disableRelatedVideos = 'disableRelatedVideos',
  /// 其他
  ...
  schemeVariant = 'schemeVariant', enableSingleRow = 'enableSingleRow', // 首页单列
  displayMode = 'displayMode', maxRowWidth = 'maxRowWidth', // 首页列最大宽度（dp）
  videoPlayerRemoveSafeArea = 'videoPlayerRemoveSafeArea', // 视频播放器移除安全边距
  videoPlayerShowStatusBarBackgroundColor = 'videoPlayerShowStatusBarBackgroundColor', // 播放页状态栏显示为背景色
  ...
  sideBarPosition = 'sideBarPosition', enableMYBar = 'enableMYBar', hideSearchBar = 'hideSearchBar', // 收起顶栏
  hideTabBar = 'hideTabBar', // 收起底栏
  tabbarSort = 'tabbarSort', // 首页tabbar
  dynamicBadgeMode = 'dynamicBadgeMode', hiddenSettingUnlocked = 'hiddenSettingUnlocked', enableGradientBg = 'enableGradientBg';
}
...
class LocalCacheKey {
  ...
  static const String historyPause = 'historyPause',
          ...
          accessKey =
          'accessKey',
          // Non-sensitive markers used to distinguish upgrades from reinstalls.
          authInstallId =
          'authInstallId',
